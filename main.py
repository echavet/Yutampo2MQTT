    ...
    try:
        ...
        if msg.topic.endswith("/set"):  # Commande de température
            try:
                ...
                LOGGER.error(f"Payload invalide pour température : {msg.payload.decode()}. Doit être un nombre.")
                return
            if not (30 <= new_temp <= 60):
                LOGGER.warning(f"Température demandée {new_temp} hors plage (30-60°C). Ignorée.")
                return
            if send_control_request(device_id, parent_id, new_temp=new_temp, new_mode=None):
            ...
            if new_mode not in ("heat", "off"):
                LOGGER.warning(f"Mode invalide : {new_mode}. Doit être 'heat' ou 'off'.")
                return
...
def update_data():
    ...
    if device_data and "data" in device_data and "elements" in device_data["data"]:
        for element in device_data["data"]["elements"]:
            ...
            # Utiliser realMode au lieu de onOff pour déterminer l'état
            mode = "heat" if real_mode == 1 else "off"
